import {
  ...
  SaleManager_v_1_3 as SaleManagerContract,
  UpdateMerkleRoot,
  UpdateMaxQueueTime,
  UpdateUri
} from "../../../generated/SaleManager_v_1_3/SaleManager_v_1_3";
...
import { getOrCreateAccount, getOrCreateNativePaymentMethod, getOrCreateTokenPaymentMethod, getSale, getSaleImplementation, hardCodedSaleUris, hardCodedSaleIds, getOrCreateDistributor } from "../../lib";
...
export function handleDeploy(event: Deploy): void {
  ...
  if (result.reverted || result.value != '1.3') {
    log.warning('v1.3 subgraph attempted to handle deployment for sale manager with version: {} in tx: {}', [result.reverted ? 'reverted' : result.value, event.transaction.hash.toHexString()]);
...
export function handleNewSale(event: NewSale): void {
  log.info('v1.3 mapping: handling new sale with address {} in tx {}', [event.address.toHexString(), event.transaction.hash.toHexString()]);
  ...
  // v1.3 saleBuyLimit uses 6 decimals, saleMaximum uses 8 decimals
  sale.saleMaximum = event.params.saleBuyLimit.times(BigInt.fromI32(10).pow(2));
  // v1.3 userBuyLimit uses 6 decimals, userMaximum uses 8 decimals
  sale.userMaximum = event.params.userBuyLimit.times(BigInt.fromI32(10).pow(2));
  // v1.3 purchaseMinimum uses 6 decimals, purchaseMinimum uses 8 decimals
  sale.purchaseMinimum = event.params.purchaseMinimum.times(BigInt.fromI32(10).pow(2));
...
export function handleRegisterClaimManager(event: RegisterClaimManager): void {
  ...
  log.warning('*** v1.3 sale manager mapping: HANDLING REGISTER CLAIM MANAGER at address {}, distributor {} *** ',[event.params.saleId.toHexString(), event.params.claimManager.toHexString()] )
  const sale = getSale(event.params.saleId.toHexString());
