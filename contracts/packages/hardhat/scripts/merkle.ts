// License: GPL3
// Reference: https://github.com/Uniswap/merkle-distributor

import { utils } from 'ethers'
import { bufferToHex, keccak256 } from 'ethereumjs-util'


// value that will be hashed into the leaf
type LeafDatum = {
  name: string,
  type: string, // TODO: restrict to allowed types (should be accessible somewhere within ethers.js)
  value: string | number
}

// input record that will be processed into a leaf on the merkle tree
type Leaf = {
  data: LeafDatum[]
}
...
export class AccountTree {
  ...
  constructor(accounts: any) {
    this.tree = new MerkleTree(
      accounts.map((account, index) => {
        return AccountTree.toNode({address: account[0], ...account[1]})
      })
    )
  }
  ...
  public static toNode(account: any): Buffer {
    const typesArray: any[] = []
    const valuesArray: any[] = []

    account.data?.forEach(dataItem => {
      typesArray.push(dataItem.type)
      valuesArray.push(dataItem.value)
    })

    const str = utils.solidityKeccak256(typesArray, valuesArray)
  ...
  public getProof(account: any): string[] {
    return this.tree.getHexProof(AccountTree.toNode(account))
  }
}

export function parseAccounts(accounts: any) {
 // construct a tree of sorted addresses
  const sortedAccounts: any = Object.keys(accounts).sort((key1, key2) => key1.localeCompare(key2)).reduce((obj, key) => {
    obj[key] = {address: key, ...accounts[key]}
    return obj
  }, {})

  const tree = new AccountTree(Object.entries(sortedAccounts))

  // generate claims
  const claims = Object.keys(sortedAccounts).reduce(
    (memo, key, index) => {
      const account = sortedAccounts[key]
      memo[key] = {
        proof: tree.getProof(account),
