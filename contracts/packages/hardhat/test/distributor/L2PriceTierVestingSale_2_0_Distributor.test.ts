...
import { GenericERC20, FakeChainlinkOracle, PriceTierVestingSale_2_0__factory, PriceTierVestingSale_2_0, FlatPriceSale, FlatPriceSaleFactory, L2OracleWithSequencerCheck, FakeSequencerUptimeFeed } from "../../typechain-types";
...
describe("PriceTierVestingSale_2_0", function () {
  beforeAll(async () => {
    ...
    partiallyVestedDistributor = await DistributorFactory.deploy(
      ...
      // end time
      2672905631,
      btcOracleWithSequencerCheck.address,
      [
  ...
  it("Initial setup matches sale correctly", async () => {
    ...
    expect(await distributor.getOracle()).toEqual(btcOracleWithSequencerCheck.address)
  ...
  it("A buyer can't claim while the sequencer is down", async () => {
    ...
    await expect(btcOracleWithSequencerCheck.latestRoundData()).rejects.toMatchObject({
    ...
    await expect(distributor.claim(buyer.address)).rejects.toMatchObject({
      reason: expect.stringMatching(/sequencerdown/i)
    })
  })
  ...
  it("A buyer can't claim within the grace period after the sequencer is up", async () => {
    ...
    expect(answer).toEqual(BigNumber.from(0))
    expect(startedAt).toEqual(BigNumber.from(await lastBlockTime()))

    const buyer = buyer0
    const distributor = partiallyVestedDistributor
    await expect(distributor.claim(buyer.address)).rejects.toMatchObject({
      reason: expect.stringMatching(/graceperiodnotover/i)
    })
  })
  ...
  it("Handles negative adjustments to a user's total claimable amount", async () => {
    ...
    expect(newAllocation).toEqual(initialAllocation.sub(10000n))
