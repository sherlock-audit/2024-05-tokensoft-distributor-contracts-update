  ...
  proof: {
    ...
    claims: {
      ...
      "0x70997970C51812dc3A010C7d01b50e0d17dc79C8": {
        proof: ["0xa4170e52dc35c1127b67e1c2f5466fce9673e61b44e077b7023f7c994d55c5dd"],
        data: [
          {
            name: "index",
            type: "uint256",
...
describe("TrancheVestingMerkle", function () {
  ...
  it("A user can claim without initialization", async () => {
    ...
    expect(await distributor.getVotes(user.address)).toEqual(
  ...
  it("reverts on misconfiguration during deployment", async () => {
    ...
    ).rejects.toMatchObject({ message: expect.stringMatching(/vesting ends after 4102444800/) });
  });
    ...
    const checkSomeTranches = async (tranches: Tranche[]) => {
      for (let i = 0; i < 10; i++) {
        ...
        if (i < newTranches.length) {
          ...
          await expect(distributor.getTranche(i)).rejects.toThrow(/reverted with panic code 0x32/);
        }
      }
    };
