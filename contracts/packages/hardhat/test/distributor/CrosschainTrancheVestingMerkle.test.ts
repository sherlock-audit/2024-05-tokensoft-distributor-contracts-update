...
import {
  GenericERC20,
  CrosschainTrancheVestingMerkle__factory,
...
describe("CrosschainTrancheVestingMerkle", function () {
  beforeAll(async () => {
    ...
    connextMockSource = await ConnextFactory.deploy(SOURCE_DOMAIN);
    connextMockDestination = await ConnextFactory.deploy(DESTINATION_DOMAIN);
    ...
    await token.transfer(distributor.target, await distributor.total());
    await token.transfer(distributorWithQueue.target, await distributorWithQueue.total());
  });
  ...
  it("Initial distributor configuration is correct", async () => {
    ...
    for (let [i, tranche] of distributorTranches.entries()) {
      expect(tranche.time).toEqual(tranches[i].time);
      expect(tranche.vestedFraction).toEqual(tranches[i].vestedFraction);
    }
  ...
  //       distributor: (await token.balanceOf(distributor.target)),
  //       connext: (await token.balanceOf(connextMockSource.target))
  //     }
  //   }
  ...
  //   expect(decodedFee.caller).toEqual(distributor.target);
  ...
  //   expect(decoded.asset).toEqual(token.target);
  ...
  //   expect(distributionRecord.claimed).toEqual(BigInt(amount) / 2n)
  ...
  it("Can claim via Merkle Proof", async () => {
    ...
    const [beneficiary, amount, domain] = config.proof.claims[`${user.address.toLowerCase()}_1735353714`].data.map(
      d => d.value,
    );
    const proof = config.proof.claims[`${user.address.toLowerCase()}_1735353714`].proof;

    // check that user can't claim with invalid proof
    const badProof = ["0xc7da9af04efd13b603589588a13c9e63474c4d79452a944db4e9c7d2d2c7db1e"];
    await expect(distributor.connect(user).claimByMerkleProof(user.address, amount, badProof)).rejects.toMatchObject({
      message: expect.stringMatching(/invalid proof/),
    });

    await distributor.connect(user).claimByMerkleProof(user.address, amount, proof);

    const balance = await token.balanceOf(user.address);
    expect(balance).toEqual(BigInt(amount) / 2n);
  ...
  it("Ineligible user cannot claim", async () => {
    ...
    const [beneficiary, amount, domain] = config.proof.claims[`${eligible3.address.toLowerCase()}_1735353714`].data.map(
      d => d.value,
    );
    ...
    await expect(
      distributor
        .connect(user)
        .claimBySignature(user.address, domain, eligible3.address, domain, amount, signature, proof),
    ).rejects.toMatchObject({ message: expect.stringMatching(/!recovered/) });

    await expect(distributor.connect(user).claimByMerkleProof(user.address, amount, proof)).rejects.toMatchObject({
      message: expect.stringMatching(/invalid proof/),
    });
  });
  ...
  it("Can claim via Connext calls", async () => {
    ...
    const [beneficiary, amount, domain] = config.proof.claims[`${eligible3.address.toLowerCase()}_2`].data.map(
      d => d.value,
    );
    const proof = config.proof.claims[`${user.address.toLowerCase()}_2`].proof;

    const transactionData = await satellite.connect(user).initiateClaim(amount, proof);

    const transactionReceipt = await transactionData.wait();
    const iface = new ethers.Interface(SatelliteDefinition.abi);

    const logs = transactionReceipt!.logs;
    const transferId = iface.parseLog(logs[1])!.args[0];

    await connextMockSource
      .connect(user)
      .callXreceive(transferId, amount, otherToken.target, user.address, 2, proof, distributor.target);
  ...
  it("Queue Delay works as expected", async () => {
    ...
    const randomValue = await distributorWithQueue.randomValue();
    ...
    expect(await distributorWithQueue.getFairDelayTime(closestAddress)).toEqual(0n);
    expect(await distributor.getFairDelayTime(closestAddress)).toEqual(0n);
    ...
    expect(await distributorWithQueue.getFairDelayTime(furthestAddress)).toEqual(maxDelayTime);

    // the furthest address should not have any delay if the queue is not enabled
    expect(await distributor.getFairDelayTime(furthestAddress)).toEqual(0n);
    ...
    for (let user of users) {
      const delay = await distributorWithQueue.getFairDelayTime(user.address);
      ...
      await distributorWithQueue.setTranches([{ time: now + 2n, vestedFraction: 10000n }]);

      const [, amount] = config.proof.claims[`${user.address.toLowerCase()}_1735353714`].data.map(d => d.value);
      const proof = config.proof.claims[`${user.address.toLowerCase()}_1735353714`].proof;

      // verify the user cannot yet claim
      await expect(distributorWithQueue.claimByMerkleProof(user.address, amount, proof)).rejects.toMatchObject({
        message: expect.stringMatching(/Distributor: no more tokens claimable right now/),
      });
      ...
      await distributorWithQueue.connect(user).claimByMerkleProof(user.address, amount, proof);
    }
  });
  ...
  //   const votingPower1 = await distributor.getVotes(eligible1.address);
  ...
  //   const votingPower2 = await distributor.getVotes(eligible1.address);
