  ...
  proof: {
    ...
    claims: {
      ...
      "0x70997970c51812dc3a010c7d01b50e0d17dc79c8": {
        proof: ["0x76b9712b2409dcb4449bd096a2902b87b13c43a5072e2da15920338790e7972d"],
        data: [
          {
            name: "beneficiary",
            type: "address",
...
describe("Satellite", function () {
  ...
  it("Metadata is correct", async () => {
    ...
    // Added .toString() to the below 2 expect's to avoid BigInt serialization issues.
    expect((await satellite.distributorDomain()).toString()).toEqual(distributorDomain.toString());
    expect((await satellite.domain()).toString()).toEqual(domain.toString());
    expect(await satellite.connext()).toEqual(connext.target);
  ...
  it("Can change owner", async () => {
    ...
    ).rejects.toMatchObject({ message: expect.stringMatching(/caller is not the owner/) });
  });
  ...
  it("Can change merkle root", async () => {
    ...
    expect(await satellite.getMerkleRoot()).toEqual(
      "0xc1778e1119d42ffb00f014fe412946116e02f73b32e324022cedb5256b5b95cd",
    );
  });
  ...
  it("Can start a cross-chain claim including a relayer fee by calling Connext", async () => {
    ...
    const transactionData = await satellite.connect(user).initiateClaim(amount, proof, { value: 1000000000000000n });
    const transactionReceipt = await transactionData.wait();
    const iface = new ethers.Interface(SatelliteDefinition.abi);
    const logs = transactionReceipt!.logs;
  ...
  it("Distributor domain must not match satellite domain", async () => {
    ...
    ).rejects.toMatchObject({ message: expect.stringMatching(/same domain/) });
  });
});
