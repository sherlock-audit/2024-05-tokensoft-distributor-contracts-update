...
describe("TrancheVestingSale_2_0", function () {
  beforeAll(async () => {
    [deployer, admin, recipient, buyer0, buyer1, buyer2, buyer3, buyer4, buyer5, nonBuyer, feeRecipient] =
  ...
  it("A buyer can claim without initialization", async () => {
    ...
    // delegate to self
    const myDistributor = await ethers.getContractAt("ContinuousVestingMerkle", distributor.target, buyer);
  ...
  it("can only be deployed when the sale is closed", async () => {
    ...
    ).rejects.toMatchObject({ message: expect.stringMatching(/TVS_2_0_D: sale not over/) });
  });
