...
describe("TrancheVestingSale_1_3", function () {
  beforeAll(async () => {
    [deployer, admin, recipient, buyer0, buyer1, buyer2, buyer3, buyer4, buyer5, nonBuyer] = await ethers.getSigners();
    ...
    saleManager = (await SaleManagerFactory.deploy(usdc.target, 6, chainlinkOracle.target)) as SaleManager_v_1_3;
    ...
    for (let signer of buyers) {
      const mySaleManager = await ethers.getContractAt("SaleManager_v_1_3", saleManager.target, signer);
      const myUSDC = await ethers.getContractAt("GenericERC20", usdc.target, signer);

      await myUSDC.approve(mySaleManager.target, usdcPaymentAmount);

      // buy with USDC
      await mySaleManager["buy(bytes32,uint256,bytes32[])"](
        publicSaleId,
        usdcPaymentAmount,
        // no merkle proof
        [],
      ...
      // buy with ETH
      await mySaleManager["buy(bytes32,bytes32[])"](
        publicSaleId,
        // no merkle proof
        [],
        {
    ...
    await saleManager.registerClaimManager(publicSaleId, partiallyVestedDistributor.target);
  ...
  it("Initial setup matches sale correctly", async () => {
    const distributor = partiallyVestedDistributor;
    expect(await distributor.saleManager()).toEqual(saleManager.target);
    ...
    const totalSpent = await saleManager.getTotalSpent(publicSaleId);
    expect(totalSpent).toEqual(BigInt(buyers.length) * 102942400n);

    // each bought token cost $1.50 and has 18 decimals instead of 6
    const totalBought = await saleManager.spentToBought(publicSaleId, totalSpent);
    ...
    const boughtTokens = (signer: SignerWithAddress) =>
      buyers.includes(signer) ? (102942400n * 10n ** 18n) / nctPrice : 0n;

    for (let user of [deployer, buyer0, buyer1, buyer2, buyer3, buyer4, buyer5, nonBuyer]) {
    ...
    expect(await distributor.total()).toEqual(totalBought);
  ...
  it("A buyer can claim without initialization", async () => {
    ...
    // delegate to self
    const myDistributor = await ethers.getContractAt("TrancheVestingSale_1_3", distributor.target, buyer);
    ...
    expectCloseEnough(await distributor.getVotes(buyer.address), (currentlyClaimable * votingFactorBips) / 10000n, 1n);
  ...
  it("reverts on misconfiguration during deployment", async () => {
    ...
    ).rejects.toMatchObject({ message: expect.stringMatching(/token decimals do not match sale/) });
    ...
    ).rejects.toMatchObject({ message: expect.stringMatching(/invalid sale id/) });
    ...
    ).rejects.toMatchObject({ message: expect.stringMatching(/last tranche must vest all tokens/) });
    ...
    ).rejects.toMatchObject({ message: expect.stringMatching(/tranche time must increase/) });
    ...
    ).rejects.toMatchObject({ message: expect.stringMatching(/tranche vested fraction must increase/) });
  });
  ...
  it("can only be deployed when the sale is closed", async () => {
    ...
    // make a purchase
    const mySaleManager = await ethers.getContractAt("SaleManager_v_1_3", saleManager.target, buyer0);
    ...
    ).rejects.toMatchObject({ message: expect.stringMatching(/TVS_1_3_D: sale not over/) });
  });
