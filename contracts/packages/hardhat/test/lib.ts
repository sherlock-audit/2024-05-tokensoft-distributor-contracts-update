...
export const getSaleId = async (tx: any): Promise<string> => {
  return (await tx.wait()).logs[0].topics[1];
};

export const getSaleAddress_2_0 = async (tx: any) => {
  const receipt = await tx.wait();

  for (let e of receipt.logs!) {
    if (e.eventName == "NewSale") {
      const newSaleEvent = e as any;
...
export const getGovernorProposalId = async (tx: any) => {
  const receipt = await tx.wait();
