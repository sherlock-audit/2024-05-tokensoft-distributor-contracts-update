...
contract FlatPriceSale is Sale, PullPaymentUpgradeable {
    ...
    event ImplementationConstructor(
        address payable indexed feeRecipient,
        uint256 feeBips
    );
  ...
  */
    function initialize(
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            require(
                oracles[i].decimals() == BASE_CURRENCY_DECIMALS,
                "token oracle decimals != 8"
            );
  ...
  */
    function _execute(
        ...
        if (data.length > 0) {
            require(
                uint8(bytes1(data[0:1])) == PER_USER_PURCHASE_LIMIT,
                "unknown data"
            );
