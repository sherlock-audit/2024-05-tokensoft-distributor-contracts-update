...
contract SaleManager_v_1_3 is ReentrancyGuard, PullPayment, ISaleManager_v_1_3 {
    ...
    modifier canAccessSale(bytes32 saleId, bytes32[] calldata proof) {
        ...
        if (sales[saleId].merkleRoot != bytes32(0)) {
            require(
                this._isAllowed(sales[saleId].merkleRoot, msg.sender, proof) ==
                    true,
        ...
        require(
            block.timestamp - sales[saleId].startTime >
                getFairQueueTime(saleId, msg.sender),
            "not your turn yet"
        );
    ...
    function getAdmin(
        bytes32 saleId
    ) public view validSale(saleId) returns (address) {
    ...
    function getMerkleRoot(
        bytes32 saleId
    ) public view validSale(saleId) returns (bytes32) {
    ...
    function getClaimManager(
        bytes32 saleId
    ) public view validSale(saleId) returns (address) {
        return (sales[saleId].claimManager);
    }

    function getSaleBuyLimit(
        bytes32 saleId
    ) public view validSale(saleId) returns (uint256) {
        return (sales[saleId].saleBuyLimit);
    }

    function getUserBuyLimit(
        bytes32 saleId
    ) public view validSale(saleId) returns (uint256) {
        return (sales[saleId].userBuyLimit);
    }

    function getPurchaseMinimum(
        bytes32 saleId
    ) public view validSale(saleId) returns (uint256) {
    ...
    function getUri(
        bytes32 saleId
    ) public view validSale(saleId) returns (string memory) {
    ...
    function getRandomValue(
        bytes32 saleId
    ) public view validSale(saleId) returns (uint160) {
        return sales[saleId].randomValue;
    }

    function getMaxQueueTime(
        bytes32 saleId
    ) public view validSale(saleId) returns (uint256) {
        return sales[saleId].maxQueueTime;
    }

    function generateRandomishValue(
        bytes32 merkleRoot
    ...
    */
        return
            uint160(uint256(blockhash(block.number - 1))) ^
            uint160(uint256(merkleRoot));
    }

    function getFairQueueTime(
        bytes32 saleId,
        address buyer
    ) public view validSale(saleId) returns (uint256) {
        /**
    ...
    */
    ...
    function isOpen(
        bytes32 saleId
    ) public view validSale(saleId) returns (bool) {
  ...
  */
    function newSale(
        ...
        require(
            userBuyLimit <= saleBuyLimit,
            "userBuyLimit cannot exceed saleBuyLimit"
        );
        require(
            purchaseMinimum <= userBuyLimit,
            "purchaseMinimum cannot exceed userBuyLimit"
        );
        require(userBuyLimit > 0, "userBuyLimit must be > 0");
        require(saleBuyLimit > 0, "saleBuyLimit must be > 0");
        require(
            endTime - startTime > maxQueueTime,
        ...
        require(
            ...
            "a sale with these parameters already exists"
        );
    ...
    function setStart(
        ...
        require(
            block.timestamp < sales[saleId].endTime,
            "disabled after sale close"
        );
        require(
            startTime < sales[saleId].endTime,
            "sale start must precede end"
        );
        require(startTime <= 4102444800, "max: 4102444800 (Jan 1 2100)");
        require(
    ...
    function setEnd(
        ...
        require(
            block.timestamp < sales[saleId].endTime,
            "disabled after sale closes"
        );
        require(endTime > block.timestamp, "sale must end in future");
        require(endTime <= 4102444800, "max: 4102444800 (Jan 1 2100)");
        require(
            sales[saleId].startTime < endTime,
            "sale must start before it ends"
        );
        require(
        ...
        nonReentrant
    {
        ...
        require(
            tokenQuantity + sales[saleId].spent[msg.sender] <=
                sales[saleId].userBuyLimit,
            "purchase exceeds your limit"
        );

        require(
            tokenQuantity + sales[saleId].totalSpent <=
                sales[saleId].saleBuyLimit,
    ...
    function registerClaimManager(
        bytes32 saleId,
        address claimManager
    ) public validSale(saleId) isAdmin(saleId) {
        require(
            claimManager != address(0),
            "Claim manager must be a non-zero address"
        );
