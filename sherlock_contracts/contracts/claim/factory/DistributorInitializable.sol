 ...
 */
abstract contract DistributorInitializable is
    Initializable,
    IDistributor,
    ReentrancyGuard
{
    ...
    function _initializeDistributionRecord(
        ...
        // Checks
        require(
            _totalAmount <= type(uint120).max,
            "Distributor: totalAmount > type(uint120).max"
        );
        uint120 totalAmount = uint120(_totalAmount);

        // Effects - note that the existing claimed quantity is re-used during re-initialization
        records[beneficiary] = DistributionRecord(
            true,
            totalAmount,
            records[beneficiary].claimed
        );
    ...
    function _executeClaim(
        ...
        uint120 claimableAmount = uint120(
            getClaimableAmount(beneficiary, data)
        );
        require(
            claimableAmount > 0,
