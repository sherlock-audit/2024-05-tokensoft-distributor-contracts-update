...
describe("PriceTierVestingSale_2_0", function () {
  ...
  it("A buyer can claim without initialization", async () => {
    ...
    expect(await distributor.getVotes(buyer.address)).toEqual((buyerTotal - currentlyClaimable) * votingFactorBips / 10000n)
    ...
    expect(await distributor.getVotes(buyer.address)).toEqual(0n)
    expect(await distributor.getVotes(delegatee.address)).toEqual((buyerTotal - currentlyClaimable) * votingFactorBips / 10000n)
